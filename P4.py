...
def modulador(bits, fc, mpp):
    ...
    # 4. Asignar las formas de onda según los bits para portadora1 y portadora2
    cont = 1
    for i, bit in enumerate(bits):
        
        if (bit == 1) and (cont%2 != 0):
...
def demodulador(senal_Rx, portadoraX, mpp):
    '''Un método que simula un bloque demodulador
...
plt.plot(time, Promedio, lw=8,color='C0',label='Promedio de realizaciones')
...
#Número de símbolos (198x89x8x3)
Ns = Nm // mpp

#Tiempo del símbolo = período de la onda portadora
Tc = 1 / fc
